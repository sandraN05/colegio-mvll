...
/* ════════════════════════════════
   HISTORIA - CARRUSEL DE IMÁGENES
════════════════════════════════ */
let historiaSlideIndex = 0;
function initHistoriaCarousel() {
  const slides = document.querySelectorAll('.historia-slide');
  if (slides.length <= 1) return;
  setInterval(function() {
    slides[historiaSlideIndex].classList.remove('activa');
    historiaSlideIndex = (historiaSlideIndex + 1) % slides.length;
    slides[historiaSlideIndex].classList.add('activa');
  }, 4000);
}
